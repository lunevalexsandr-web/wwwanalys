...
- **`template_indicator.py`** - модель показателя в шаблоне (ссылка на библиотеку)
...
6. **Интеграция с 1С** - импорт справочника показателей из 1С Предприятие
7. **Импорт/Экспорт** - загрузка и выгрузка показателей (CSV, JSON, Excel)
